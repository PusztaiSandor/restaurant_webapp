    ...
    <form action="{{ route('global-charges.update', $charge->global_charges_id) }}" method="POST">
      ...
      <div class="mb-3">
        ...
        <select name="charge_type" id="charge_type" class="form-select" required>
          <option value="delivery_fee" {{ $charge->charge_type === 'delivery_fee' ? 'selected' : '' }}>Szállítási
            díj</option>
          <option value="service_fee" {{ $charge->charge_type === 'service_fee' ? 'selected' : '' }}>Szervízdíj
          </option>
          <option value="order_discount" {{ $charge->charge_type === 'order_discount' ? 'selected' : '' }}>
            Rendelési kedvezmény</option>
          <option value="cutlery" {{ $charge->charge_type === 'cutlery' ? 'selected' : '' }}>Evőeszköz díj
          </option>
        </select>
      </div>
      ...
      <div class="mb-3">
        ...
        <select name="delivery_method" id="delivery_method" class="form-select" required>
          <option value="delivery" {{ $charge->delivery_method === 'delivery' ? 'selected' : '' }}>Kiszállítás
          </option>
          <option value="dine-in" {{ $charge->delivery_method === 'dine-in' ? 'selected' : '' }}>Helyben
            fogyasztás</option>
          <option value="pickup" {{ $charge->delivery_method === 'pickup' ? 'selected' : '' }}>Személyes átvétel
          </option>
        </select>
      </div>
