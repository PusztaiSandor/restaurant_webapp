...
@section('content')
  <div class="container">
    ...
    <form method="GET" class="row mb-4">
      ...
      <div class="col-12 col-md-2">
        ...
        <select name="sort" id="sort" class="form-select">
          ...
          <option value="price_asc" {{ request('sort') === 'price_asc' ? 'selected' : '' }}>Ár szerint növekvő
          </option>
          <option value="price_desc" {{ request('sort') === 'price_desc' ? 'selected' : '' }}>Ár szerint csökkenő
          </option>
        </select>
      </div>
    ...
    <a href="{{ route('menu.pdf') }}" class="btn btn-outline-primary mb-4" title="Nyomtatható étlap letöltése PDF-ben"
      target="_blank">
        ...
        <div class="col-md-4 mb-4">
          <div class="card h-100 shadow-sm d-flex flex-column" id="dish_{{ $dish->dishes_id }}">
            ...
            <div class="card-body d-flex flex-column">
              ...
              <div class="mt-auto">
                @if ($dish->stock > 0)
                  <a href="{{ route('dishes.show', ['dish' => $dish->dishes_id]) }}"
                    class="btn btn-sm btn-success w-100 mb-2">Részletek és rendelés</a>
      ...
      @foreach ($dishes as $dish)
        ...
        if (discountPercent > 0 && originalDisplay && discountDisplay) {
          ...
          discountDisplay.innerHTML =
            `Eredeti ár: <del id="original_${dishId}">${formatPrice(gross)}</del> • Kedvezmény: –${discountPercent}%`;
        }
