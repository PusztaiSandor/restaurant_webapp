      ...
      @foreach ($orders as $order)
        <div class="card mb-3">
          ...
          <div class="card-body">
            ...
            @if ($order->is_paid)
              <a href="{{ route('order.invoice', $order->orders_id) }}" class="btn btn-outline-secondary btn-sm mb-2"
                target="_blank" title="Számla letöltése PDF-ben">
              ...
              <div class="mt-3 p-3 border rounded bg-light">
                ...
                <form method="POST" action="{{ route('order.rate', $order->orders_id) }}">
                  ...
                  <div class="mb-2">
                    ...
                    <textarea name="rating_comment" id="rating_comment" class="form-control" rows="2" maxlength="300"
                      placeholder="Pl. Finom volt minden étel."></textarea>
                    <small class="form-text text-muted">
                      Opcionális. Maximum 300 karakter. HTML, szkript és speciális karakterek nem
                      engedélyezettek.
                    </small>
                  </div>

                  <button type="submit" class="btn btn-outline-success btn-sm">Értékelés
                    mentése</button>
                </form>
              </div>
              ...
              <div class="mt-3 p-3 border rounded bg-light">
                ...
                @if ($order->booking)
                  ...
                  @if (in_array($order->status, ['uj', 'keszul', 'atvetelre_kesz']) &&
                          ...
                          !$order->is_paid)
                    <form method="POST" action="{{ route('bookings.cancel', $order->booking->bookings_id) }}"
                      onsubmit="return confirm('Biztosan lemondod az asztalfoglalást?');">
                      @csrf
            ...
            @if (in_array($order->status, ['uj', 'keszul', 'atvetelre_kesz']) && !$order->is_paid)
              <form method="POST" action="{{ route('order.cancel', $order->orders_id) }}" class="mt-2"
                onsubmit="return confirm('Biztosan törölni szeretnéd ezt a rendelést?');">
                @csrf
