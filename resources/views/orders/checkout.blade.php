  ...
  <div class="container">
    ...
    @guest
      <div class="alert alert-warning">
        A rendelés leadásához <a href="{{ route('login') }}">jelentkezz be</a> vagy <a
          href="{{ route('register') }}">regisztrálj</a>.
      </div>
    @endguest
    ...
    <form method="POST" action="{{ route('order.submit') }}">
        ...
        <ul class="list-group mb-3" id="charge-list">
          @foreach ($charges as $charge)
            @php
              ...
              $amount = $charge['is_percentage'] ? round($baseTotal * ($charge['value'] / 100), 2) : $charge['value'];
            @endphp
      ...
      <div class="text-end">
        ...
        @else
          <button type="button" class="btn btn-secondary" disabled>Megrendelés elküldése (bejelentkezés
            szükséges)</button>
        @endauth
      </div>
    </form>
