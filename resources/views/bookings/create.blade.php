          ...
          @foreach ($tables as $table)
            <div class="col-md-4 mb-2">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" name="table_code[]" value="{{ $table->table_code }}"
                  id="table_{{ $table->tables_id }}">
                <label class="form-check-label" for="table_{{ $table->tables_id }}">
                  {{ strtoupper($table->table_code) }} – {{ ucfirst($table->location) }},
                  {{ ucfirst($table->position) }} ({{ $table->capacity }} fő)
                  @if ($table->notes)
