          ...
          <div class="dish">
            <div class="dish-row">
              ...
              <div class="dish-price">
                @if ($dish->on_sale)
                  <div class="dish-price">
                    <div class="original-price">{{ number_format($dish->gross_price, 0, ',', ' ') }}
                      Ft</div>
                    <div style="color: #d9534f; font-size: 11px;">Kedvezmény:
                      −{{ $dish->discount_percent }}%</div>
                    <div class="price">
                      {{ number_format($dish->gross_price * (1 - $dish->discount_percent / 100), 0, ',', ' ') }}
                      Ft</div>
                  </div>
                @else
                  <div class="dish-price">
                    <div class="price">{{ number_format($dish->gross_price, 0, ',', ' ') }} Ft
                    </div>
                  </div>
                @endif
              </div>
            ...
            <div class="dish-details">
              ...
              @if (!empty($dish->allergens))
                | Allergének:
                {{ is_array($dish->allergens) ? implode(', ', $dish->allergens) : $dish->allergens }}
              @endif
            </div>
